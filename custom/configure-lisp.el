...
(defun def-elisp-abbrev (trigger expansion &optional hook)
  (define-abbrev emacs-lisp-mode-abbrev-table trigger expansion (and hook #'elisp-abbrev-hook)))

(defun elisp-abbrev-hook ()
  "Run after elisp abbrev is expanded. Search back for $ char and place cursor there."
  (when (re-search-backward "\\$" nil t)
    (delete-char)))

(defvar elisp-abbrevs
  '(("ah" "add-hook '")
    ("atl" "add-to-list '")
    ("bb" "bury-buffer")
    ("bc" "forward-char -1")
    ...
    ("bp" "boundp '")
    ("bs" "forward-symbol -1")
    ("bus" "buffer-substring-no-properties ")
    ("buss" "buffer-substring ")
    ("bw" "forward-word -1")
    ("con" "concat ")
    ("ca" "char-after")
    ("cb" "current-buffer")
    ...
    ("mt" "mapconcat ")
    ("mu" "multiple-value-bind ()" t)
    ("mv" "multiple-value-bind ()" t)
    ...
    ("rsb" "re-search-backward \"\" t nil 'noerror")
    ("rsf" "re-search-forward \"\" t nil 'noerror")
    ("sa" "symbol-at-point")
    ("sap" "symbol-at-point")
...
(dolist (abbrev elisp-abbrevs)
  (apply #'def-elisp-abbrev abbrev))
